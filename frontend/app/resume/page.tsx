  ...
  return (
    <div className="w-full relative min-h-screen py-10 px-6 overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-10">
        ...
        {/* ── Hero Header ── */}
        <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-7">
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
            <div className="space-y-3">
              ...
              {/* Contact chips */}
              <div className="flex flex-wrap gap-2 text-[11px] font-semibold text-slate-600 dark:text-zinc-400">
                <a href={'mailto:' + CONTACT.email} className="flex items-center gap-1.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-3 py-1.5 rounded-full hover:border-brand-cyan/50 hover:text-brand-cyan transition">
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  {CONTACT.email}
                </a>
                <a href={'tel:' + CONTACT.phone} className="flex items-center gap-1.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-3 py-1.5 rounded-full hover:border-brand-cyan/50 hover:text-brand-cyan transition">
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  {CONTACT.phone}
                </a>
                <a href={CONTACT.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-3 py-1.5 rounded-full hover:border-brand-purple/50 hover:text-brand-purple transition">
                ...
                <a href={CONTACT.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-3 py-1.5 rounded-full hover:border-blue-400/50 hover:text-blue-400 transition">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
                  LinkedIn
                </a>
                <span className="flex items-center gap-1.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 px-3 py-1.5 rounded-full">
            ...
            <a
              ...
              className="shrink-0 bg-brand-purple hover:bg-brand-purple-dark text-zinc-900 dark:text-white text-xs font-bold px-5 py-3 rounded-2xl transition active:scale-[0.98] shadow-lg flex items-center gap-2"
            >
          ...
          {sections.map(s => (
            <button
              ...
              className={
                ...
                (activeSection === s.id
                  ? 'bg-brand-purple text-zinc-900 dark:text-white border-brand-purple shadow-lg shadow-brand-purple/20'
                  : 'bg-slate-50 dark:bg-zinc-950/60 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-800 hover:border-brand-purple/40 hover:text-zinc-900 dark:text-white')
              }
            >
              {s.label}
            </button>
        ...
        {activeSection === 'experience' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="relative border-l-2 border-slate-300 dark:border-zinc-900 pl-6 ml-3.5 space-y-10">
                {timelineEntries.map((entry, idx) => (
                  <div key={idx} className="relative group">
                    <span className="absolute -left-[32px] top-1.5 w-4 h-4 rounded-full bg-slate-50 dark:bg-zinc-950 border-2 border-brand-purple group-hover:bg-brand-purple transition duration-300" />
                    <div className="space-y-2">
                      ...
                      <h3 className="text-base font-extrabold text-zinc-900 dark:text-white">{entry.role}</h3>
                ...
                {education.map((edu, idx) => (
                  <div key={idx} className="relative group">
                    <span className="absolute -left-[32px] top-1.5 w-4 h-4 rounded-full bg-slate-50 dark:bg-zinc-950 border-2 border-brand-cyan group-hover:bg-brand-cyan transition duration-300" />
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-xs font-semibold flex-wrap gap-1">
                        <span className="text-brand-cyan">{edu.duration}</span>
                        <span className="text-zinc-550">{edu.location}</span>
                      </div>
                      <h3 className="text-base font-extrabold text-zinc-900 dark:text-white">{edu.degree}</h3>
            ...
            <div className="lg:col-span-5 space-y-8">
              <div className="space-y-4">
                ...
                <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-4">
                  <SkillRadar />
                </GlassCard>
              </div>
              ...
              <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-5 space-y-4">
                <h3 className="text-xs font-extrabold uppercase text-slate-600 dark:text-zinc-400 border-b border-slate-300 dark:border-zinc-900 pb-2">Core Proficiency</h3>
                ...
                <GlassCard key={idx} hoverEffect className={'border ' + project.color + ' !p-0 overflow-hidden'}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-2">
                        <h3 className="text-base font-extrabold text-zinc-900 dark:text-white">{project.name}</h3>
                      ...
                      <svg
                        className={'w-4 h-4 shrink-0 mt-1 text-slate-500 dark:text-zinc-500 transition-transform duration-300 ' + (expandedProject === idx ? 'rotate-180' : '')}
                  ...
                  {expandedProject === idx && (
                    <div className="px-5 pb-5 border-t border-slate-200 dark:border-zinc-800/50">
                      <ul className="space-y-2 text-xs leading-relaxed text-slate-600 dark:text-zinc-400 font-medium list-disc list-inside mt-4">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skillGroups.map((group, idx) => (
                <GlassCard key={idx} hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-5 space-y-4">
                  <h3 className={'text-xs font-extrabold uppercase tracking-wider border-b border-slate-300 dark:border-zinc-900 pb-2 flex items-center gap-2 ' + group.color}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {group.skills.map((skill, si) => (
                      <span
                        key={si}
                        className="text-[11px] font-bold px-3 py-1.5 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-slate-700 dark:text-zinc-300 hover:border-brand-purple/40 hover:text-zinc-900 dark:text-white transition cursor-default"
                      >
                        {skill}
                      </span>
                    ))}
            ...
            {/* Proficiency bars */}
            <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-6 space-y-5">
              <h3 className="text-xs font-extrabold uppercase text-slate-600 dark:text-zinc-400 border-b border-slate-300 dark:border-zinc-900 pb-2">Proficiency Levels</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {coreSkills.map((skill, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex justify-between text-xs font-semibold">
                      <span className="text-slate-700 dark:text-zinc-300">{skill.name}</span>
                      <span className="text-brand-cyan">{skill.level}%</span>
                    </div>
        ...
        {activeSection === 'education' && (
          <div className="space-y-6">
            ...
            {education.map((edu, idx) => (
              <GlassCard key={idx} hoverEffect={false} className="border-brand-cyan/20 bg-slate-50 dark:bg-zinc-950/60 !p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1.5">
                    <h3 className="text-base font-extrabold text-zinc-900 dark:text-white">{edu.degree}</h3>
                    <p className="text-sm text-slate-600 dark:text-zinc-400 font-semibold">{edu.institution}</p>
                    <p className="text-xs text-zinc-550">{edu.location}</p>
                  </div>
                  <div className="text-right space-y-2">
                    <p className="text-xs font-bold text-slate-500 dark:text-zinc-500">{edu.duration}</p>

...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!isAdmin) {
      alert('Access restricted. Please sign in to open the admin workspace.');
      router.push('/login');
    }
  }, [isAdmin]);
    ...
    <div className="w-full relative min-h-screen py-10 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-zinc-900 light:border-slate-200 pb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white light:text-slate-900">Admin Hub</h1>
            <p className="text-xs text-zinc-550 mt-1">Manage your developer universe with modular ledgers.</p>
          </div>

          <button
            onClick={() => setSeederOpen(true)}
            className="bg-brand-purple hover:bg-brand-purple-dark text-white text-xs font-bold px-4.5 py-2.5 rounded-xl transition active:scale-[0.98] shadow-md flex items-center gap-1.5"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 relative overflow-hidden">
            <span className="absolute top-2 right-2 text-zinc-800 text-3xl font-bold font-mono opacity-15">01</span>
            <div className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider">Total Posts</div>
            <div className="text-2xl font-black text-white mt-1.5">{metrics.totalPosts}</div>
          ...
          <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 relative overflow-hidden">
            ...
            <div className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider">Project Views</div>
          ...
          <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 relative overflow-hidden">
            ...
            <div className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider">Pending Mod</div>
          ...
          <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 relative overflow-hidden">
            ...
            <div className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider">Active Quizzes</div>
            <div className="text-2xl font-black text-white mt-1.5">{metrics.activeQuizzes}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 border-b lg:border-b-0 lg:border-r border-zinc-900 light:border-slate-200 pr-0 lg:pr-6">
            <button
              onClick={() => setActiveTab('overview')}
              className={`w-full text-left text-xs font-bold p-3 rounded-xl transition ${activeTab === 'overview' ? 'bg-brand-purple text-white' : 'text-zinc-500 hover:bg-zinc-900/40 hover:text-white'}`}
            >
              📊 Hub Overview
            </button>
            <button
              onClick={() => setActiveTab('projects')}
              className={`w-full text-left text-xs font-bold p-3 rounded-xl transition ${activeTab === 'projects' ? 'bg-brand-purple text-white' : 'text-zinc-500 hover:bg-zinc-900/40 hover:text-white'}`}
            >
              📁 Manage Projects
            </button>
            <button
              onClick={() => setActiveTab('posts')}
              className={`w-full text-left text-xs font-bold p-3 rounded-xl transition ${activeTab === 'posts' ? 'bg-brand-purple text-white' : 'text-zinc-500 hover:bg-zinc-900/40 hover:text-white'}`}
            >
              📝 Manage Blogs & Docs
            </button>
            <button
              onClick={() => setActiveTab('comments')}
              className={`w-full text-left text-xs font-bold p-3 rounded-xl transition ${activeTab === 'comments' ? 'bg-brand-purple text-white' : 'text-zinc-500 hover:bg-zinc-900/40 hover:text-white'}`}
            >
              💬 Comment Moderation
            </button>
            <button
              onClick={() => setActiveTab('settings')}
              className={`w-full text-left text-xs font-bold p-3 rounded-xl transition ${activeTab === 'settings' ? 'bg-brand-purple text-white' : 'text-zinc-500 hover:bg-zinc-900/40 hover:text-white'}`}
            >
              ⚙️ Site Settings
            </button>
          </div>
          ...
          <div className="lg:col-span-9 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="relative max-w-md w-full">
                  ...
                  <input
                    ...
                    className="w-full text-xs bg-zinc-900/60 light:bg-slate-100 border border-zinc-850 light:border-slate-200 focus:border-brand-purple focus:outline-none rounded-full pl-10 pr-4 py-2.5 text-white light:text-slate-900"
                  />
                </div>
                ...
                <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5">
                  <h3 className="text-xs font-extrabold uppercase text-zinc-400 mb-4 tracking-wider">Recent Activity</h3>
                      ...
                      <tbody className="divide-y divide-zinc-900/60">
                        {recentActivities.map((act, idx) => (
                          <tr key={idx} className="hover:bg-white/2">
                            <td className="py-4 font-bold text-white light:text-slate-800">{act.name}</td>
                            <td className="py-4 text-zinc-500">{act.type}</td>
                            <td className="py-4">
                            ...
                            <td className="py-4 text-zinc-500 text-right">{act.date}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {/* Maintenance Card */}
                <GlassCard hoverEffect={false} className="bg-gradient-to-tr from-brand-indigo/10 to-brand-purple/5 border border-zinc-900/80 !p-5">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <h4 className="text-xs font-extrabold uppercase text-white tracking-widest">Demo Data Import Wizard</h4>
                    ...
                    <button
                      onClick={() => setSeederOpen(true)}
                      className="bg-brand-purple hover:bg-brand-purple-dark text-white text-xs font-extrabold px-4.5 py-2.5 rounded-xl transition active:scale-[0.98] shadow-md"
                    >
                      Start Import
                    </button>
                  </div>
            ...
            {activeTab === 'projects' && (
              <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-900 pb-3">
                  <h3 className="text-xs font-extrabold uppercase text-zinc-400 tracking-wider">Project Ledgers</h3>
                  <Link href="/" className="text-[10px] font-bold text-brand-purple hover:underline">+ Add Project</Link>
                </div>

                <div className="space-y-3">
                  {projects.map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3.5 bg-zinc-900/40 rounded-xl border border-zinc-900 text-xs">
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => { deleteProject(p.id); alert('Project entry deleted.'); }}
            ...
            {activeTab === 'posts' && (
              <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-900 pb-3">
                  <h3 className="text-xs font-extrabold uppercase text-zinc-400 tracking-wider">Case Studies & Documentation</h3>
                ...
                <div className="space-y-3">
                  {posts.map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3.5 bg-zinc-900/40 rounded-xl border border-zinc-900 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => { deletePost(p.id); alert('Article post deleted.'); }}
                          className="text-red-500 hover:underline"
                        >
                          Delete
                        </button>
            ...
            {activeTab === 'comments' && (
              <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 space-y-4">
                <h3 className="text-xs font-extrabold uppercase text-zinc-400 tracking-wider border-b border-zinc-900 pb-3">
                    ...
                    {comments.map(c => (
                      <div
                        ...
                        className={`p-4 rounded-xl border text-xs space-y-2
                          ${c.isFlagged ? 'bg-red-500/5 border-red-500/10' : !c.isApproved ? 'bg-yellow-500/5 border-yellow-500/10' : 'bg-zinc-900/30 border-zinc-900'}
                        `}
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-extrabold text-white">{c.authorName} ({c.authorEmail})</span>
                          <span className="text-[9px] text-zinc-650">{c.date}</span>
                        </div>

                        <p className="text-zinc-400 italic">"{c.content}"</p>

                        <div className="flex items-center gap-3 pt-2 border-t border-zinc-900/50 text-[10px] font-bold">
                          {!c.isApproved && (
                            <button
            ...
            {/* SETTINGS TAB */}
            {activeTab === 'settings' && (
              <GlassCard hoverEffect={false} className="border-zinc-900/60 bg-zinc-950/60 !p-5 space-y-5">
                <h3 className="text-xs font-extrabold uppercase text-zinc-400 tracking-wider border-b border-zinc-900 pb-3">
                ...
                <div className="space-y-4 text-xs font-semibold">
                  ...
                  <div>
                    ...
                    <div className="p-3 bg-zinc-900/40 rounded-xl border border-zinc-900 text-zinc-500 leading-normal">

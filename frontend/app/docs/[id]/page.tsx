...
export default function BlogDetail({ params }: PageProps) {
  ...
  if (!post) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
        ...
        <p className="text-slate-500 dark:text-zinc-500 text-sm mt-2">The requested write-up does not exist in our system database.</p>
  ...
  const handleSubmitComment = (e: React.FormEvent) => {
    ...
    addComment('post', post.id, finalName, finalEmail, commentContent);
    setAuthorName('');
    setAuthorEmail('');
    setCommentContent('');
    toast.info('Thank you! Your discussion thread is submitted and awaiting moderator approval.');
  };
  ...
  return (
    <div className="w-full relative min-h-screen py-10 px-6 overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto space-y-12 relative z-10">
        ...
        <section className="space-y-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-zinc-500">{post.readTime}</span>
          </div>

          <h1 className="text-3xl md:text-4xl lg:text-5xl font-black tracking-tight text-zinc-900 dark:text-white light:text-slate-900 leading-tight">
            {post.title}
          </h1>

          <p className="text-sm md:text-base text-slate-600 dark:text-zinc-400 light:text-slate-650 leading-relaxed font-semibold">
            {post.subtitle}
          </p>

          {/* Author metadata block */}
          <div className="flex items-center gap-3 border-t border-b border-slate-300 dark:border-zinc-900 light:border-slate-200 py-4 mt-8">
            <div>
              <div className="text-xs font-extrabold text-zinc-900 dark:text-white light:text-slate-800">By {post.author}</div>
              <div className="text-[10px] text-zinc-550 mt-0.5">Published on {post.date}</div>
            </div>

            {/* Inline Upvote button */}
            <button
              className="flex items-center gap-1.5 p-2 px-3 rounded-xl bg-white dark:bg-zinc-900/60 hover:bg-white dark:bg-zinc-900 text-[10px] font-bold border border-zinc-850 text-brand-purple hover:text-brand-cyan transition ml-auto"
              onClick={() => { likePost(post.id); toast.success('Upvoted article successfully!'); }}
            >
          ...
          {/* Hero visual image */}
          <div className="w-full aspect-video rounded-3xl overflow-hidden border border-slate-200 dark:border-zinc-800/40 shadow-2xl relative">
            <img
              src={post.image}
              alt={post.title}
        ...
        <section className="prose prose-invert max-w-none text-sm md:text-base leading-relaxed text-slate-700 dark:text-zinc-300 light:text-slate-650 space-y-6">
          <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
            ...
            <GlassCard hoverEffect={false} className="bg-gradient-to-tr from-brand-purple/15 to-transparent border border-brand-purple/20">
              <h3 className="text-sm font-bold text-zinc-900 dark:text-white">Quiz Challenge</h3>
              <p className="text-[10px] text-slate-600 dark:text-zinc-400 mt-1 leading-normal">Take our CSS Architecture quiz to test your knowledge.</p>
              <Link
                href="/games"
                className="inline-flex items-center gap-1 text-[10px] font-extrabold bg-brand-purple text-zinc-900 dark:text-white px-3.5 py-1.5 rounded-lg mt-4 active:scale-[0.98] transition"
              >
                Start Quiz →
              </Link>
            </GlassCard>
            ...
            {/* <GlassCard hoverEffect={false} className="bg-gradient-to-tr from-brand-cyan/15 to-transparent border border-brand-cyan/20">
              <h4 className="text-sm font-extrabold text-zinc-900 dark:text-white">Watch the Implementation</h4>
              <p className="text-[10px] text-slate-600 dark:text-zinc-400 mt-1 leading-normal">See a live-coding session where we build these frosted glass effects from scratch.</p>
              <button
                className="w-12 h-12 rounded-full bg-brand-purple hover:bg-brand-purple-dark text-zinc-900 dark:text-white flex items-center justify-center transition shadow-[0_0_15px_rgba(139,92,246,0.5)] transform hover:scale-110 active:scale-95"
                onClick={() => toast.info('Launching video canvas presentation...')}
              >
        ...
        <section className="border-t border-slate-300 dark:border-zinc-900 light:border-slate-200 pt-12 space-y-8" id="comment-form">

          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white light:text-slate-900">Discussion ({postComments.length})</h3>
          ...
          <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/40">
            <h4 className="text-xs font-extrabold uppercase text-slate-600 dark:text-zinc-400 mb-4 tracking-wider">
              {currentUser ? `Join the conversation as ${currentUser.name}...` : 'Join the conversation...'}
            </h4>
            <form onSubmit={handleSubmitComment} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-xs bg-white dark:bg-zinc-900/60 light:bg-slate-100 border border-zinc-850 light:border-slate-250 rounded-xl px-3.5 py-2.5 text-zinc-900 dark:text-white light:text-slate-900 focus:outline-none focus:border-brand-purple"
                    />
                  </div>
                  <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-xs bg-white dark:bg-zinc-900/60 light:bg-slate-100 border border-zinc-850 light:border-slate-250 rounded-xl p-3.5 text-zinc-900 dark:text-white light:text-slate-900 focus:outline-none focus:border-brand-purple"
                />
              </div>
              ...
              <div className="flex items-center justify-end text-[10px] text-zinc-650 font-semibold gap-4">
                ...
                <button
                  type="submit"
                  className="bg-brand-purple hover:bg-brand-purple-dark text-zinc-900 dark:text-white font-extrabold px-6 py-2.5 rounded-xl text-xs active:scale-[0.98] transition shadow-md"
                >
                  Post Comment
                </button>
              </div>
        ...
        <section className="border-t border-slate-300 dark:border-zinc-900 light:border-slate-200 pt-12 space-y-6">
          <h3 className="text-lg font-extrabold text-zinc-900 dark:text-white light:text-slate-900">Suggested Reading</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recommendedPosts.map(p => (
              ...
              >
                ...
                <h4 className="text-sm font-extrabold text-zinc-900 dark:text-white light:text-slate-900 mt-1 leading-snug line-clamp-1">{p.title}</h4>
                <p className="text-[11px] text-slate-500 dark:text-zinc-500 mt-1 line-clamp-2 leading-relaxed">{p.subtitle}</p>
              </GlassCard>
            ))}
          </div>
        </section>
      ...
      <div className="md:hidden fixed bottom-16 left-0 right-0 z-30 bg-slate-50 dark:bg-zinc-950/95 light:bg-white/95 backdrop-blur-md border-t border-slate-300 dark:border-zinc-900 light:border-slate-150 py-3.5 px-6 flex items-center justify-between text-slate-600 dark:text-zinc-400 light:text-slate-500 shadow-2xl">
        <button
          className="flex-1 flex items-center justify-center gap-2 p-3 bg-white dark:bg-zinc-900 hover:bg-zinc-850 text-zinc-900 dark:text-white text-xs font-bold rounded-xl transition border border-slate-200 dark:border-zinc-800 hover:border-brand-purple group"
          onClick={() => { likePost(post.id); toast.success('Upvoted successfully!'); }}
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-bold text-slate-600 dark:text-zinc-400"
        >

      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* Category tabs */}
            <div className="flex items-center gap-1 bg-white dark:bg-zinc-900/60 light:bg-slate-100 p-1.5 rounded-2xl border border-zinc-850 light:border-slate-200 w-fit">
              <button
                onClick={() => setCategoryFilter('all')}
                className={`text-xs font-bold px-4 py-2 rounded-xl transition ${categoryFilter === 'all' ? 'bg-brand-purple text-zinc-900 dark:text-white' : 'text-slate-500 dark:text-zinc-500 hover:text-zinc-350'}`}
              >
                All Posts
              </button>
              <button
                onClick={() => setCategoryFilter('blog')}
                className={`text-xs font-bold px-4 py-2 rounded-xl transition ${categoryFilter === 'blog' ? 'bg-brand-purple text-zinc-900 dark:text-white' : 'text-slate-500 dark:text-zinc-500 hover:text-zinc-350'}`}
              >
                Blog Articles
              </button>
              <button
                onClick={() => setCategoryFilter('docs')}
                className={`text-xs font-bold px-4 py-2 rounded-xl transition ${categoryFilter === 'docs' ? 'bg-brand-purple text-zinc-900 dark:text-white' : 'text-slate-500 dark:text-zinc-500 hover:text-zinc-350'}`}
              >
                Documentation
              </button>
            </div>
            ...
            {filteredPosts.length === 0 ? (
              ...
              <div className="space-y-6">
                {filteredPosts.map((post) => (
                  <GlassCard
                    key={post.id}
                    className="flex flex-col border-slate-300 dark:border-zinc-900/60 transition group hover:border-brand-purple/20"
                    onClick={() => window.location.href = `/docs/${post.id}`}
                  >
                    <div className="flex flex-col md:flex-row gap-6">

                      {/* Image panel */}
                      <div className="md:w-1/3 aspect-video md:aspect-square rounded-2xl overflow-hidden border border-slate-300 dark:border-zinc-900 bg-slate-50 dark:bg-zinc-950">
                        <img
                          src={post.image}
                          alt={post.title}
                      ...
                      <div className="md:w-2/3 flex flex-col justify-between space-y-4">
                        <div className="space-y-2.5">
                          ...
                          <h3 className="text-base md:text-lg font-extrabold text-zinc-900 dark:text-white light:text-slate-900 group-hover:text-brand-purple transition leading-snug">
                            {post.title}
                          </h3>
                        ...
                        {/* Interactive counts */}
                        <div className="flex items-center justify-between mt-4 border-t border-slate-300 dark:border-zinc-900/60 light:border-slate-100 pt-4 text-[10px] font-bold text-slate-500 dark:text-zinc-500">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-5">
              <div className="flex items-center gap-3 border-b border-slate-300 dark:border-zinc-900 pb-4 mb-4">
                <img
                ...
                />
                <div>
                  <h4 className="text-xs font-extrabold text-zinc-900 dark:text-white">Felix De</h4>
            ...
            {/* Trending Tags Panel */}
            <GlassCard hoverEffect={false} className="border-slate-300 dark:border-zinc-900/60 bg-slate-50 dark:bg-zinc-950/60 !p-5">
              <h4 className="text-xs font-extrabold tracking-wider uppercase text-zinc-450 mb-3 border-b border-slate-300 dark:border-zinc-900 pb-2">
                Trending Categories
              </h4>
                ...
                {allTags.map((tag) => (
                  <button
                    ...
                    className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border transition
                      ${selectedTag === tag ? 'bg-brand-purple text-zinc-900 dark:text-white border-brand-purple' : 'bg-white dark:bg-zinc-900 light:bg-slate-100 hover:bg-zinc-850 text-zinc-450 light:text-slate-650 border-zinc-850 light:border-slate-200'}
                    `}
                  >
                    {tag}
                  </button>
              ...
              <div className="space-y-4">
                {trendingPosts.map((post) => (
                  <Link
                    key={post.id}
                    href={`/docs/${post.id}`}
                    className="block group border-b border-slate-300 dark:border-zinc-900/60 pb-3 last:border-0 last:pb-0"
                  >
            ...
            {/* RSS / Subscribe Card */}
            {/* <GlassCard hoverEffect={false} className="bg-gradient-to-tr from-brand-indigo/10 to-brand-purple/5 border border-slate-300 dark:border-zinc-900/80 !p-5 relative overflow-hidden shadow-lg">
              <h4 className="text-xs font-extrabold uppercase text-zinc-900 dark:text-white">Subscribe to RSS</h4>
              ...
              <form
                onSubmit={(e) => { e.preventDefault(); toast.success('Subscribed to newsletter feeds successfully!'); }}
              ...
              >
                <input
                  ...
                  className="bg-white dark:bg-zinc-900 text-[10px] rounded-lg border border-zinc-850 p-2 text-zinc-900 dark:text-white w-full focus:outline-none placeholder-zinc-650"
                />
                <button
                  type="submit"
                  className="bg-brand-purple hover:bg-brand-purple-dark text-zinc-900 dark:text-white text-[10px] font-extrabold px-3 py-2 rounded-lg"
                >
                  Join
                </button>
              </form>
